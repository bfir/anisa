...
tab_panel, tab_agente = st.tabs(["Panel de pacientes", "Preguntar a Anisa"])
...
with tab_panel:
    ...
    if not nombre:
        ...
        if not pacientes:
            ...
            c4.metric("Pasaporte", paciente["pasaporte"])
...
with tab_agente:
    st.subheader("Pregúntale a Anisa")
    ...
    for turno in st.session_state.historial_chat:
        with st.chat_message(turno["rol"]):
            st.write(turno["contenido"])
            if turno.get("pasos"):
                with st.expander(f"Ver {len(turno['pasos'])} paso(s) del agente"):
                    for paso in turno["pasos"]:
                        st.write(f"🔧 **{paso['herramienta']}**({paso['argumentos']})")
                        st.json(paso["resultado"])

    pregunta = st.chat_input("Escribe tu pregunta...")
    if pregunta:
        st.session_state.historial_chat.append({"rol": "user", "contenido": pregunta})
        with st.spinner("Pensando..."):
            respuesta = requests.post(f"{API}/agente", json={"pregunta": pregunta}).json()
